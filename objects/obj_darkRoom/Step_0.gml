...
if (surface_exists(surf)){
	...
	#region //obj_itemPickup Redraw
		...
		var vh = camera_get_view_y(view_camera[0])+270-64;
			if pickupPrompt == true{
				...
				draw_text_ext_transformed_colour(camera_get_view_x(view_camera[0])+(480/2),camera_get_view_y(view_camera[0])+270-(270/8), "There is a " + string(itemName) + ", do I want it?",3,1280,.4,.4,0,c_white,c_white,c_white,c_white,1); //draw the text at the coordinates
	...
	*/
	...
	with obj_textDirectorNeo{
		#region //obj_mouse Redraw
		
	draw_roundrect_ext(x+18-q,y+20+q,x+110-10+q,y+200-16-q,8,8,false);
	draw_circle(obj_player.x+(-sign(obj_player.x-obj_mouse.x)*3),obj_player.y-6,n,false);
	#endregion
	}
